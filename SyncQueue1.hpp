...
class SyncQueue1 {
    private:
        ...
        int add(F&& x){
         ...
         if(!m_not_full_.wait_for(locker,std::chrono::seconds(m_waittime_,[this]{return needstop_||!isFull()}))){
            return 1;
         }
         if(needstop_){
            return 2;
...
template <typename T>
inline bool SyncQueue1<T>::isFull() const
{
    std::unique_lock<std::mutex> lock(mutex_);
    if(size_ >= max_size_)
        return true;    
...
template <typename T>
inline bool SyncQueue1<T>::isEmpty() const
{
    std::unique_lock<std::mutex> lock(mutex_);
    if(size_ == 0)
        return true;
